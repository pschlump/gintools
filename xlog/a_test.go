...
func TestXLog(t *testing.T) {
	...
	r.Use(XLog(utcLogger, time.RFC3339, true))

	localLogger, localLoggerObserved := buildDummyLogger()
	r.Use(XLog(localLogger, time.RFC3339, false))
...
func TestXLogWithConfig(t *testing.T) {
	r := gin.New()

	utcLogger, utcLoggerObserved := buildDummyLogger()
	r.Use(XLogWithConfig(utcLogger, &Config{
