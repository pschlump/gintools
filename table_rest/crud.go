...
func HandleStoredProcedureConfig(c *gin.Context, SPData *CrudStoredProcConfig) {
	...
	case "GET", "POST": // select
		...
		if err != nil {
			...
			c.Writer.WriteHeader(http.StatusNotAcceptable) // 406
			fmt.Fprintf(c.Writer, "Error fetching return data from %s ->%s<- error %s at %s\n", SPData.StoredProcedureName, stmt, err, dbgo.LF())
			return
		}
		...
		if err != nil {
			fmt.Fprintf(logFilePtr, "Error 406: %s %s\n", err, dbgo.LF())
			c.Writer.WriteHeader(http.StatusNotAcceptable) // 406
			fmt.Fprintf(c.Writer, "Error parsing return data ->%s<-form %s ->%s<- error %s at %s\n", SPData.StoredProcedureName, stmt, rawData, err, dbgo.LF())
			return
		}
		if getStatus.Status != "success" {
			...
			fmt.Fprintf(c.Writer, "Error data ->%s<-form %s ->%s<- error %s at %s\n", SPData.StoredProcedureName, stmt, rawData, err, dbgo.LF())
			return
		}
		SetJSONHeaders(c)
...
func HandleCRUDPerTableRequests(c *gin.Context, CrudData *CrudConfig) {
	...
	case "GET": // select
		...
		if err == nil {
			...
			c.Writer.WriteHeader(http.StatusNotAcceptable) // 406
			fmt.Fprintf(c.Writer, `{"status":"error":"msg":%q}`, err)
		}
		return
	...
	case "POST": // insert
		...
		if err != nil {
			...
			// xyzzy5 - TODO - Must have error text returned. -- this is a general error - all error returns must have a message.
			SetJSONHeaders(c)
			fmt.Fprintf(c.Writer, `{"status":"error","msg":%q}`, "Error inserting data.")
			// xyzzy - end
			c.Writer.WriteHeader(http.StatusNotAcceptable) // 406
			return
		}
